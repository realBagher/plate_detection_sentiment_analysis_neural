...
plate_detector = YOLO(
    "/mnt/i/#Repos/plate_detection_sentiment_analysis_neural/plate_detection/training/yolo12_detection_best.pt")  # Detection model
plate_reader = YOLO(
    "/mnt/i/#Repos/plate_detection_sentiment_analysis_neural/plate_detection/training/yolo12_recognition_best.pt")  # Recognition model
...
def process_image(image_path, font_size):
    ...
    try:
        font = ImageFont.truetype("/mnt/i/#Repos/plate_detection_sentiment_analysis_neural/plate_detection/app/fonts/Vazir-Bold.ttf", font_size)
    except IOError:
...
def image_processor(image_path, output_dir):
...
def process_frame(frame, font_size=30):
    """
    Processes a single video frame (BGR image) using the same detection/recognition pipeline.
    Returns the combined recognized text and the annotated frame.
    """
    image_copy = frame.copy()
    image_pil = Image.fromarray(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)).convert("RGBA")
    draw = ImageDraw.Draw(image_pil)
    detection_results = plate_detector(frame)
    predicted_texts = []
    try:
        font = ImageFont.truetype("./Vazir-Bold.ttf", font_size)
    except IOError:
        print("Could not load the specified font. Using default font.")
        font = ImageFont.load_default()
    for detection in detection_results:
    ...
    return combined_text, annotated_frame


def video_processor(video_path, output_dir, font_size=30):
    """
    Processes a video file by reading it frame-by-frame, applying plate detection/recognition,
    and writing out a new annotated video.
    """
    ...
    while True:
        ...
        if not ret:
            break
        _, annotated_frame = process_frame(frame, font_size)
        out.write(annotated_frame)
        frame_count += 1
    cap.release()
    out.release()
    print(f"Processed video saved as {output_path} with {frame_count} frames.")
